...
from util.visualizer import visualize_output
...
if __name__ == '__main__':
    ...
    vis_time = time.time()          # visualizer time
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        with tqdm(total=len(dataset)) as progress_bar:
            for i, data in enumerate(dataset):  # inner loop within one epoch
                ...
                epoch_loss_G += model.loss_G
                epoch_loss_D += model.loss_D
                epoch_loss_total += model.loss_G + model.loss_D

                if opt.visualize_model_output and (time.time()-vis_time>opt.vis_time):
                    outputs, inputs, labels = model.fake_B, data["A"], data["B"]
                    visualize_output(outputs, inputs, labels)
                    vis_time=time.time()
