...
The final Kaggle submission score was obtained by averaging over the outputs of the experiments DeepLabv3 with BCE (see Polybox Model Store: `deeplab_trainaug_testaug`), Focal (see Polybox Model Store Name: `deeplab_focal`), and Dice (see Polybox Model Store: `deeplab_dice`) loss over all 3 seeds. Either reproduce the submission scores from scratch according to steps introduced above or directly download the experiment folders from our [Polybox Model Store](https://polybox.ethz.ch/index.php/s/qtn4FY23P8lj4xG?path=%2FExperiments) and store the folders in your local Model Store.
