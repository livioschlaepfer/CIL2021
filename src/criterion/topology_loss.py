...
class TopoLoss(nn.Module):
    """
    Implementation of focal loss based on https://arxiv.org/abs/1906.05404

    """

    def __init__(self, pers_thresh=0.03, pers_thresh_perfect=0.99, topo_size=100):
    ...
    def forward(self, input, target):
        ...
        for i, (img,tar) in enumerate(zip(input, target)):
            likelihood = torch.sigmoid(img[0]).clone()
            gt = tar[0].clone()
