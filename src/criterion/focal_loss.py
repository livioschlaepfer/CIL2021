...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha=0.1, gamma=2, smooth=1e-6):
    ...
    def forward(self, input, target):
        ...
        p_1 = input * target
        p_0 = (1. - input) * ((target -1) * -1)
        p_t = p_1 + p_0

        p_t = torch.clamp(p_t, self.smooth, 1. - self.smooth)

        a_1 = self.alpha * target
        a_0 = (1-self.alpha) * ((target-1)*-1)
        a_t = a_1 + a_0

        focal_loss = -1*a_t * torch.pow((1 - p_t), self.gamma) * torch.log(p_t)
